...
namespace Escc.AddressAndPersonalDetails.Controls
{
    ...
    public class ContactInfoControl : WebControl, INamingContainer
    {
        ...
        private static void AddString(string xhtml, Control parentControl, string hCardClass)
        {
            ...
            {
                using (var span = new HtmlGenericControl("span"))
                {
        ...
        protected override void CreateChildControls()
        {
            ...
            using (HtmlGenericControl part1 = new HtmlGenericControl("p"))
            {
                using (HtmlGenericControl part2 = new HtmlGenericControl("p"))
                {
                    ...
                    if (this.description != null && this.description.Length > 0)
                    {
                        ...
                        if (fnFound)
                        {
                            ContactInfoControl.AddString(
                                HttpUtility.HtmlEncode(this.Description).Replace(Environment.NewLine, "<br />"), part1);
                        }
                        else
                        {
                            ContactInfoControl.AddString(
                    ...
                    if (this.bs7666Address != null)
                    {
                        ...
                        if (this.bs7666Address.HasAddress())
                        {
                            ...
                            if (this.showMap)
                            {
                                ...
                                {
                                    ContactInfoControl.StartOnNewLine(part1);
                                    using (var mapLink = new HtmlAnchor())
                                    {
                                        mapLink.InnerText = this.MapLinkText;
                                        mapLink.HRef = HttpUtility.HtmlEncode(mapPageUrl);
                                        mapLink.Attributes["data-unpublished"] = "false";
                                        // disable website CMS warning about internal links
                    ...
                    foreach (var phone in this.telephones)
                    {
                        ...
                        try
                        {
                            ...
                            ContactInfoControl.AddString(ExpandAbbreviations(FormatTelephone(phone.NationalNumber)),
                                telContainer, "value");
                    ...
                    if (this.fax != null && this.fax.Length > 0)
                    {
                        ...
                        ContactInfoControl.AddString(ExpandAbbreviations(FormatTelephone(this.fax)), faxContainer,
                            "value");
                    }
                    ...
                    if (this.emailAddress != null && this.emailAddress.Length > 0)
                    {
                        ...
                        {
                            emailLinkText = HttpUtility.HtmlEncode(Resources.EmailPrefix + " " + this.emailText);
                        }
                        ...
                        if (visibleAddress && !Mobile)
                            ContactInfoControl.AddString(Resources.EmailPrefix + Resources.ContactPrefixSeparator, part2);
                        string emailLink = "<a href=\"" + emailHref + "\"";
                        if (!String.IsNullOrEmpty(emailClass)) emailLink += " class=\"" + emailClass + "\"";
                        emailLink += ">" + emailLinkText + "</a>";
        ...
        private static string GetWebsiteMapUrl(BS7666Address address, string displayName, Uri returnUrl)
        {
            ...
            if (String.IsNullOrEmpty(address.AdministrativeArea) || String.Compare(address.AdministrativeArea.ToUpperInvariant(), "EAST SUSSEX", true, CultureInfo.CurrentCulture) != 0) return null;
            if (address.Town != null && address.Town.Length > 0 && address.Town.ToUpperInvariant().IndexOf("BRIGHTON", StringComparison.Ordinal) > -1) return null;
        ...
        private static string FormatTelephone(string tel)
        {
            var num = new UKContactNumber();
